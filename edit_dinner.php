...
if (isset($_POST['title'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        $query = "UPDATE dinner SET title = ?, content = ?, date = ?, image = ?, time = ? WHERE id = ?";
        ...
        $query = "UPDATE dinner SET title = ?, content = ?, date = ?, time = ? WHERE id = ?";
        if ($stmt = $connect->prepare($query)) {
...
if (isset($_GET['id'])) {
    ...
    if ($stm = $connect->prepare("SELECT * FROM dinner WHERE id = ?")) {

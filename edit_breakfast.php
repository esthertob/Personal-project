...
if (isset($_POST['title'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        $query = "UPDATE breakfast SET title = ?, content = ?, date = ?, image = ?, time = ? WHERE id = ?";
        if ($stmt = $connect->prepare($query)) {
            $stmt->bind_param('sssssi', $_POST['title'], $_POST['content'], $_POST['date'], $destination,  $_POST['time'], $_GET['id']);
            $stmt->execute();
            $stmt->close();
        ...
        $query = "UPDATE breakfast SET title = ?, content = ?, date = ?, time = ? WHERE id = ?";
        if ($stmt = $connect->prepare($query)) {
            $stmt->bind_param('ssssi', $_POST['title'], $_POST['content'], $_POST['date'], $_POST['time'], $_GET['id']);
            $stmt->execute();
            $stmt->close();
...
if (isset($_GET['id'])) {
    ...
    if ($stm = $connect->prepare("SELECT * FROM breakfast WHERE id = ?")) {
